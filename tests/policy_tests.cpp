...
void testRunModes() {
    ...
    expect(play_policy::shouldCaptureStartposSegment(RunMode::Startpos), "capture startpos segment");
...
void testNewBestPolicy() {
    ...
    expect(play_policy::newBestToNotify(best) == std::optional<int>{45}, "death snapshot new best");
...
void testLevelFilter() {
    ...
    expect(
        level_filter::parseMode("Anything") == level_filter::Mode::All, "unknown filter mode is All"
    );
...
void testTextPolicy() {
    ...
    expect(text_policy::formatDurationMs(1000) == "1 second", "whole second milliseconds duration");

    std::string const utf8 = std::string("A") + "\xC3\xA9" + "B";
    expect(text_policy::clampUtf8ByBytes(utf8, 4) == utf8, "UTF-8 unchanged at limit");
    expect(
        text_policy::clampUtf8ByBytes(utf8, 3) == std::string("A") + "\xC3\xA9",
        "UTF-8 exact code point boundary"
    );
    ...
    expect(!play_policy::segmentMeetsThreshold(81, 100, 20), "startpos completion below threshold");
}
...
void testRetryPolicy() {
    ...
    expect(
        retry_policy::delayForFailure(429, "7", 0, 3) == std::optional<int>{7}, "Retry-After honored"
    );
    expect(
